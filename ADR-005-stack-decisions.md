...
**Статус:** Частично заменён (см. ADR-006)
**Дата:** 2026-04-27
**Контекст сессии:** session 02, после двух волн разведки
**Зависит от:** ADR-001, ADR-002, ADR-003, ADR-004
**Изменён:** ADR-006 (pivot на Mindcraft + Python middleware + Andy-4)

> **Внимание:** конкретный стек, описанный ниже, был пересмотрен после разведки путей реализации с учётом 8GB VRAM. Замены: PuppetPlayers → Mindcraft+Mineflayer, Qwen2.5-7B → Andy-4, свой Fabric mod → Python middleware. См. ADR-006 для актуального стека. Обоснования и альтернативы из этого ADR сохраняют ценность как контекст принятого решения.
