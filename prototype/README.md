...
### 2. Скачать модель — `t-tech/T-lite-it-2.1`
Qwen3-8B base, не-think вариант (без `<think>` блоков), нативно русский, ~5 GB Q4_K_M. См. ADR-006 R2 для контекста: Andy-4 (Llama-8B-R1 fine-tune) был отвергнут — он лил reasoning в чат, ломал персону, навязчиво звал Mindcraft-команды.
...
Проверить:
```cmd
ollama run t-tech/T-lite-it-2.1:Q4_K_M "Привет"
```
(Ctrl+C для выхода)
...
```
Применить наши патчи к Mindcraft (vision-стабы + нейтрализация self_prompter):
```cmd
..\living-npcs\prototype\scripts\windows\patch_mindcraft.bat
```
Без этого Mindcraft каждые 2 секунды шлёт боту «Your next response MUST contain a command» — это превращает Living NPC в goal-driven assistant и противоречит миссии проекта (см. README §«Принципы»). Патч обратим: оригиналы лежат рядом как `*.orig`.
